...
class MysqliDb
{
	...
	{
		//$this->_query = filter_var($query, FILTER_SANITIZE_STRING);	//commentd by wjdi
		$this->_query = $query;
	...
	public function rawInsert($sql)
	{
		...
		return ($stmt->affected_rows > 0 ? $stmt->affected_rows : false);
	}

	/*
		write by wjdi.
		use preparedStatement for update.
		@return  false 或者 影响行数（可能为0）
	 */
	public function rawUpdate($sql, $bindParams=null)
	{
	...
	public function update($tableName, $tableData)
	{
		...
		if (empty($this->_where)) {
			die("cannot update with no WHERE");
		}
